# -*- coding: utf-8 -*-
"""Copy of Welcome To Colaboratory

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1GtLu3hSFZeK_Y9n8k0R54xrlro_YcpCR

<div class="markdown-google-sans">
  <h1>Welcome to Colab!</h1>
</div>

If you're already familiar with Colab, check out this video to learn about interactive tables, the executed code history view, and the command palette.

<center>
  <a href="https://www.youtube.com/watch?v=rNgswRZ2C1Y" target="_blank">
  <img alt='Thumbnail for a video showing 3 cool Google Colab features' src="data:image/png;base64," height="188" width="336">
  </a>
</center>

<div class="markdown-google-sans">
  <h2>What is Colab?</h2>
</div>

Colab, or "Colaboratory", allows you to write and execute Python in your browser, with
- Zero configuration required
- Access to GPUs free of charge
- Easy sharing

Whether you're a **student**, a **data scientist** or an **AI researcher**, Colab can make your work easier. Watch [Introduction to Colab](https://www.youtube.com/watch?v=inN8seMm7UI) to learn more, or just get started below!

<div class="markdown-google-sans">

## **Getting started**
</div>

The document you are reading is not a static web page, but an interactive environment called a **Colab notebook** that lets you write and execute code.

For example, here is a **code cell** with a short Python script that computes a value, stores it in a variable, and prints the result:
"""

seconds_in_a_day = 24 * 60 * 60
seconds_in_a_day

"""To execute the code in the above cell, select it with a click and then either press the play button to the left of the code, or use the keyboard shortcut "Command/Ctrl+Enter". To edit the code, just click the cell and start editing.

Variables that you define in one cell can later be used in other cells:
"""

seconds_in_a_week = 7 * seconds_in_a_day
seconds_in_a_week

"""Colab notebooks allow you to combine **executable code** and **rich text** in a single document, along with **images**, **HTML**, **LaTeX** and more. When you create your own Colab notebooks, they are stored in your Google Drive account. You can easily share your Colab notebooks with co-workers or friends, allowing them to comment on your notebooks or even edit them. To learn more, see [Overview of Colab](/notebooks/basic_features_overview.ipynb). To create a new Colab notebook you can use the File menu above, or use the following link: [create a new Colab notebook](http://colab.research.google.com#create=true).

Colab notebooks are Jupyter notebooks that are hosted by Colab. To learn more about the Jupyter project, see [jupyter.org](https://www.jupyter.org).

<div class="markdown-google-sans">

## Data science
</div>

With Colab you can harness the full power of popular Python libraries to analyze and visualize data. The code cell below uses **numpy** to generate some random data, and uses **matplotlib** to visualize it. To edit the code, just click the cell and start editing.
"""

import numpy as np
from matplotlib import pyplot as plt

ys = 200 + np.random.randn(100)
x = [x for x in range(len(ys))]

plt.plot(x, ys, '-')
plt.fill_between(x, ys, 195, where=(ys > 195), facecolor='g', alpha=0.6)

plt.title("Sample Visualization")
plt.show()

"""You can import your own data into Colab notebooks from your Google Drive account, including from spreadsheets, as well as from Github and many other sources. To learn more about importing data, and how Colab can be used for data science, see the links below under [Working with Data](#working-with-data).

<div class="markdown-google-sans">

## Machine learning
</div>

With Colab you can import an image dataset, train an image classifier on it, and evaluate the model, all in just [a few lines of code](https://colab.research.google.com/github/tensorflow/docs/blob/master/site/en/tutorials/quickstart/beginner.ipynb). Colab notebooks execute code on Google's cloud servers, meaning you can leverage the power of Google hardware, including [GPUs and TPUs](#using-accelerated-hardware), regardless of the power of your machine. All you need is a browser.

Colab is used extensively in the machine learning community with applications including:
- Getting started with TensorFlow
- Developing and training neural networks
- Experimenting with TPUs
- Disseminating AI research
- Creating tutorials

To see sample Colab notebooks that demonstrate machine learning applications, see the [machine learning examples](#machine-learning-examples) below.

<div class="markdown-google-sans">

## More Resources

### Working with Notebooks in Colab

</div>

- [Overview of Colaboratory](/notebooks/basic_features_overview.ipynb)
- [Guide to Markdown](/notebooks/markdown_guide.ipynb)
- [Importing libraries and installing dependencies](/notebooks/snippets/importing_libraries.ipynb)
- [Saving and loading notebooks in GitHub](https://colab.research.google.com/github/googlecolab/colabtools/blob/main/notebooks/colab-github-demo.ipynb)
- [Interactive forms](/notebooks/forms.ipynb)
- [Interactive widgets](/notebooks/widgets.ipynb)

<div class="markdown-google-sans">

<a name="working-with-data"></a>
### Working with Data
</div>

- [Loading data: Drive, Sheets, and Google Cloud Storage](/notebooks/io.ipynb)
- [Charts: visualizing data](/notebooks/charts.ipynb)
- [Getting started with BigQuery](/notebooks/bigquery.ipynb)

<div class="markdown-google-sans">

### Machine Learning Crash Course

<div>

These are a few of the notebooks from Google's online Machine Learning course. See the [full course website](https://developers.google.com/machine-learning/crash-course/) for more.
- [Intro to Pandas DataFrame](https://colab.research.google.com/github/google/eng-edu/blob/main/ml/cc/exercises/pandas_dataframe_ultraquick_tutorial.ipynb)
- [Linear regression with tf.keras using synthetic data](https://colab.research.google.com/github/google/eng-edu/blob/main/ml/cc/exercises/linear_regression_with_synthetic_data.ipynb)

<div class="markdown-google-sans">

<a name="using-accelerated-hardware"></a>
### Using Accelerated Hardware
</div>

- [TensorFlow with GPUs](/notebooks/gpu.ipynb)
- [TensorFlow with TPUs](/notebooks/tpu.ipynb)

<div class="markdown-google-sans">

<a name="machine-learning-examples"></a>

### Featured examples

</div>

- [NeMo Voice Swap](https://colab.research.google.com/github/NVIDIA/NeMo/blob/stable/tutorials/VoiceSwapSample.ipynb): Use Nvidia's NeMo conversational AI Toolkit to swap a voice in an audio fragment with a computer generated one.

- [Retraining an Image Classifier](https://tensorflow.org/hub/tutorials/tf2_image_retraining): Build a Keras model on top of a pre-trained image classifier to distinguish flowers.
- [Text Classification](https://tensorflow.org/hub/tutorials/tf2_text_classification): Classify IMDB movie reviews as either *positive* or *negative*.
- [Style Transfer](https://tensorflow.org/hub/tutorials/tf2_arbitrary_image_stylization): Use deep learning to transfer style between images.
- [Multilingual Universal Sentence Encoder Q&A](https://tensorflow.org/hub/tutorials/retrieval_with_tf_hub_universal_encoder_qa): Use a machine learning model to answer questions from the SQuAD dataset.
- [Video Interpolation](https://tensorflow.org/hub/tutorials/tweening_conv3d): Predict what happened in a video between the first and the last frame.
"""



import plotly.graph_objects as go

# Create a simple scatter plot
scatter_fig = go.Figure(go.Scatter(x=[1, 2, 3], y=[4, 5, 6]))

# Update the layout with various parameters
scatter_fig.update_layout(
    title='My Scatter Plot',
    xaxis_title='X-axis',
    yaxis_title='Y-axis',
    xaxis=dict(
        title='Custom X-axis Title',
        showgrid=True,
        zeroline=True,
        showline=False
    ),
    yaxis=dict(
        title='Custom Y-axis Title',
        showgrid=True,
        zeroline=True,
        showline=False
    ),
    width=800,
    height=600,
    #margin=dict(l=50, r=50, t=50, b=50),
    template='plotly_dark'
)

# Show the plot
scatter_fig.show()

import pandas as pd
import numpy as np
import seaborn as sns

timesdata=pd.read_csv("/content/timesData.csv")

timesdata

timesdata.info()

timesdata.isnull().sum()

for col in timesdata.describe(include='number').columns:
  print(col)

timesdata['country'].value_counts(normalize=True).round(3).head(60)

import plotly.express as px
for col in timesdata.describe(exclude='number').columns:
  print(col)

timesdata['female_male_ratio'].isnull().sum()*100/timesdata.shape[0]

df=timesdata.iloc[:100,:]

df['country'].value_counts()

"""line plot in plotly express"""

fig=px.line(df,x='teaching',y='research',template='plotly_dark',height=500,width=1000,color='country')
fig.update_traces()
fig.update_xaxes(title_text="teaching")
fig.update_yaxes(title_text="research")
fig.update_layout(title_text="teaching vs research",title_x=0.5)
fig.show()





"""ploty in sactterplot  in the form plotly express and all paameters in scatter block only or if you want specify axis and y axis indiviually we wil above one"""

import plotly.express as px

fig=px.scatter(df,x='teaching',y='num_students',height=1000,width=1000,color='country', labels={"x": "Teaching", "y": "Number of Students"},title="teaching vs num_students",template="plotly_dark")
fig.show()

df.head(2)



"""line in plotly graph objects"""

import plotly as py
from plotly.offline import iplot


df = timesdata.iloc[:100,:]

# import graph objects as "go"
import plotly.graph_objs as go

# Creating trace1
trace1 = go.Scatter(
                    x = df.world_rank,
                    y = df.citations,
                    mode="lines",
                    name = "citations",
                    marker = dict(color ='green'),
                    text= df.university_name)
# Creating trace2
trace2 = go.Scatter(
                    x = df.world_rank,
                    y = df.teaching,
                    mode= "lines+markers",
                    name = "teaching",
                    marker = dict(color = 'red'),
                    text= df.university_name)
data = [trace1, trace2]
layout = dict(title_text = 'Citation and Teaching vs World Rank of Top 100 Universities',title_x=0.5,
              xaxis= dict(title= 'World Rank',ticklen= 5,zeroline=False)
             )
fig = dict(data = data, layout = layout)
iplot(fig)

timesdata['female_male_ratio'].value_counts()

df.head(2)

year2011=timesdata[timesdata['year']==2011].iloc[:100,:]
year2011.head(2)

year2011.shape

year2013=timesdata[timesdata['year']==2013].iloc[:100,:]
year2013.head(2)

year2013.shape



timesdata['year'].value_counts()

"""scatterplot plot graph objects"""

from plotly.offline import iplot
traces1=go.Scatter(x=year2011['teaching'],y=year2011['income'],mode='markers',marker=dict(color='red'),name='jagan')
traces2=go.Scatter(x=year2011['teaching'],y=year2011['research'],mode='markers',marker=dict(color='green'),name='jagan2')
data=[traces1,traces2]
layout=dict(title="year vs total score",
            xaxis=dict(title="year",showgrid=True,showline=True),
            yaxis=dict(title="total_score",showgrid=True,showline=True),
            template='plotly_dark'
)
fig=dict(data=data,layout=layout)
iplot(fig)

df2014 = timesdata[timesdata.year == 2014].iloc[:100,:]

tarce=go.Scatter(x=df2014['citations'],y=df2014['total_score'],mode='markers',marker=dict(color='red'),name='jagan',text='country')

trace1=go.Scatter(x=df2014['citations'],y=df2014['num_students'],mode='markers',marker=dict(color='green'),name='jagan1',text='country')
data=[tarce,trace1]
layout=dict(title="citations vs toatal score",
            xaxis=dict(title="citations",showgrid=True,showline=True),
            yaxis=dict(title="total_score",showgrid=True,showline=True),
            width=1000,
            height=1000,
            template='plotly_dark')
fig=dict(data=data,layout=layout)
iplot(fig)

df2014

"""we will select 100 rows who are have the country is united states of amercia"""

country1=timesdata[timesdata['country']=='United States of America'].iloc[:100,:]
country1.head(10)
country2=timesdata[timesdata['country']==' United Kingdom'].iloc[:100,:]
country2

country1.shape

country2=timesdata[timesdata['country']=='United Kingdom'].iloc[:100,:]
country2

t1=go.Scatter(x=country1['teaching'],y=country1['income'],mode='markers',marker=dict(color='red',size=10,symbol='circle'),name='n',text=country1['university_name'])

t2=go.Scatter(x=country2['teaching'],y=country2['international_students'],mode='markers',marker=dict(color='blue',size=15,symbol='circle'),name='n2',text=country1['university_name'])
data=[t1,t2]
layout=dict(title_text="tecahing vs income",
xaxis=dict(title="teaching",showgrid=True,showline=True),
yaxis=dict(title="income",showgrid=True,showline=True),
            width=700,
            height=1000,
            template="plotly_dark")
fig=dict(data=data,layout=layout)
iplot(fig)

"""below code apply fot the above code few things reach to the makerrsize,color,size above ones"""

import plotly.graph_objects as go
from plotly.offline import iplot

# Assuming you have a DataFrame named year2011 with columns 'teaching' and 'income'
import plotly.graph_objects as go
from plotly.offline import iplot

# Assuming you have a DataFrame named year2011 with columns 'teaching' and 'income'

trace1 = go.Scatter(
    x=year2011['teaching'],
    y=year2011['income'],
    mode='markers',  # Set mode to 'markers' for a scatter plot
    name='jagan',
    text=year2011['university_name'],  # Assuming you have a column named 'university_name' for text labels
    marker=dict(
        size=10,  # Marker size
        color='blue',  # Marker color
        symbol='circle',  # Marker symbol
        line=dict(
            color='black',  # Marker outline color
            width=2  # Marker outline width
        )
    ),
    line=dict(
        color='red',  # Line color
        width=5  # Line width
    )
)

data = [trace1]
layout = dict(title="Year vs Total Score",
template="plotly_dark")

fig = dict(data=data, layout=layout)

iplot(fig)

university1=timesdata[timesdata['university_name']=="Harvard University"].iloc[:100,:]

university2=timesdata[timesdata['university_name']=='California Institute of Technology'].iloc[:100,:]

"""Sscatterplot"""

t1=go.Scatter(x=country1['university_name'],
              y=country1['income'],
              mode='markers',
              marker=dict(color='red',size=20,symbol='circle'),

            name='n',
          text=country1['university_name'])

t2=go.Scatter(x=country2['university_name'],y=country2['international_students'],mode='markers',marker=dict(color='blue',size=15,symbol='circle'),name='n2',text=country1['university_name'])
data=[t1,t2]

layout=dict(title="country vs income",
            xaxis=dict(title="yuniversiy name",showgrid=True,showline=True),
            yaxis=dict(title="international students",showgrid=True,showline=True),
            width=1000,
            height=1000,
            template="plotly_dark")
fig=dict(data=data,layout=layout)
iplot(fig)

# prepare data frames
df2014 = timesdata[timesdata.year == 2014].iloc[:100,:]
df2015 = timesdata[timesdata.year == 2015].iloc[:100,:]
df2016 = timesdata[timesdata.year == 2016].iloc[:100,:]
# import graph objects as "go"
import plotly.graph_objs as go
# creating trace1
trace1 =go.Scatter(
                    x = df2014.world_rank,
                    y = df2014.citations,
                    mode = "markers",
                    name = "2014",
                    marker = dict(color = 'rgba(255, 128, 255, 0.8)'),
                    text= df2014.university_name)
# creating trace2
trace2 =go.Scatter(
                    x = df2015.world_rank,
                    y = df2015.citations,
                    mode = "markers",
                    name = "2015",
                    marker = dict(color = 'rgba(255, 128, 2, 0.8)'),
                    text= df2015.university_name)
# creating trace3
trace3 =go.Scatter(
                    x = df2016.world_rank,
                    y = df2016.citations,
                    mode = "markers",
                    name = "2016",
                    marker = dict(color = 'rgba(0, 255, 200, 0.8)'),
                    text= df2016.university_name)
data = [trace1, trace2, trace3]
layout = dict(title = 'Citation vs world rank of top 100 universities with 2014, 2015 and 2016 years',
              xaxis= dict(title= 'World Rank',ticklen= 5,zeroline= False),
              yaxis= dict(title= 'Citation',ticklen= 5,zeroline= False)
             )
fig = dict(data = data, layout = layout)
iplot(fig)

"""scatter plot i will follow and i will solve all problem by this algorithm"""

Scatter Example: Citation vs world rank of top 100 universities with 2014, 2015 and 2016 years

Import graph_objs as go
Creating traces
x = x axis
y = y axis
mode = type of plot like marker, line or line + markers
name = name of the plots
marker = marker is used with dictionary.
color = color of lines. It takes RGB (red, green, blue) and opacity (alpha)
text = The hover text (hover is curser)
data = is a list that we add traces into it
layout = it is dictionary.
title = title of layout
x axis = it is dictionary
title = label of x axis
ticklen = length of x axis ticks
zeroline = showing zero line or not
y axis = it is dictionary and same with x axis
fig = it includes data and layout
iplot() = plots the figure(fig) that is created by data and layout

#timesdata.groupby('university_name')['num_students3'].mean()

# prepare data frames
df2014 = timesdata[timesdata.year == 2014].iloc[:3,:]
# import graph objects as "go"
import plotly.graph_objs as go
# create trace1
trace1 = go.Bar(
                x = df2014.university_name,
                y = df2014.citations,
                name = "citations",
                marker = dict(color = 'rgba(255, 174, 255, 0.5)',
                             line=dict(color='rgb(0,0,0)',width=1.5)),
                text = df2014.country)
# create trace2
trace2 = go.Bar(
                x = df2014.university_name,
                y = df2014.teaching,
                name = "teaching",
                marker = dict(color = 'rgba(255, 255, 128, 0.5)',
                              line=dict(color='rgb(0,0,0)',width=1.5)),
                text = df2014.country)
data = [trace1, trace2]
layout =dict(title_text="university name vs citations vs teaching in 2014 an d2015 ",title_x=0.5,barmode = "group")
fig = dict(data = data, layout = layout)
iplot(fig)

dfstudent=timesdata.sort_values(by='num_students',ascending=False).iloc[:10,:]
dfstudent

dfstudent1=timesdata.sort_values(by='num_students',ascending=False).iloc[:10,:]
dfstudent1.head(10)

t1=go.Bar(x=dfstudent['university_name'],y=dfstudent['income'],text=dfstudent['num_students'],name="citations")

t2=go.Bar(x=dfstudent['university_name'],y=dfstudent['total_score'],text=dfstudent['num_students'],name='teaching')
data=[t1,t2]
layout=go.Layout(title_text="university vs student")
fig=go.Figure(data=data,layout=layout)
iplot(fig)

"""highest number of student by the university"""

data=timesdata[timesdata['country']=='United States of America']

data.sort_values(by='num_students',ascending=False)

data['teaching'] = data['teaching'].round().astype(int)

"""in tecahing greater than 80 to display us universites names"""

data1=data[data['teaching']>=75]
data1
data1[data['country']=='United States of America'][['country','university_name','num_students','international_students']].reset_index()[:70]

data3.dropna()

data2=data1[data1['country']=='United States of America'].iloc[:10]
data3=data2.sort_values(by='teaching',ascending=False)
#data3.dropna(inplace=True)
data3

data3.dropna(inplace=True)

data3.isnull().sum()

"""******bar in plotly graph object whever the if you select the rows that coresponding rows "null value" check it once such type of values whole impact on output"""

import plotly.graph_objects as go
from plotly.offline import iplot
t1=go.Bar(x=data3.university_name,
         y=data3.num_students,name='jagan')
t2=go.Bar(x=data3.university_name,
         y=data3.income,name='jagan2')

data=[t1,t2]
layout=go.Layout(title="top 20 university  in us",
                 xaxis=dict(title="university",showgrid=True,showline=True),
                 yaxis=dict(title="students",showgrid=True,showline=True),
                 width=1000,
                 height=1000,
                 template="plotly_dark")

fig=go.Figure(data=data,layout=layout)
fig.show()



"""text and name added and markers in the form barmode is group
here barmode useful for the display side by side
"""

import plotly.graph_objects as go
from plotly.offline import iplot
t1=go.Bar(x=data3.university_name,
         y=data3.num_students,name='jagan',marker=dict(color='red'))
t2=go.Bar(x=data3.university_name,
         y=data3.teaching,name='jagan2',text=data3.teaching,marker=dict(color='green'))


data = [t1,t2]

layout = go.Layout(
    title_text="Top 20 Universities in the US",barmode='group',

    xaxis=dict(title='University Name', showgrid=True, showline=True),
    yaxis=dict(title='Number of Students', showgrid=True, showline=True),
    height=1000,
    width=1000,
    template='plotly_dark'
)

fig = go.Figure(data=data, layout=layout)
fig.show()

"""ba"""

df2014 = timesdata[timesdata.year == 2014].iloc[:3,:]
df2014

"""barmode output display look look like stacked

when you apply barmode is relative it will gives to you output is stacked bar one after one
stack look like in data strcuture scenario
"""

data2=data1[data1['country']=='United States of America'].iloc[:4]
data3=data2.sort_values(by='teaching',ascending=False)
#data3.dropna(inplace=True)
data3

t=go.Bar(x=data3.country,
         y=data3.total_score)
t1=go.Bar(x=data3.country,
          y=data3.international)
data=[t,t1]
layout=go.Layout(title="country vs international guys",barmode="relative",
            xaxis=dict(title='country',showgrid=True,showline=True),
            yaxis=dict(title='total_score',showgrid=True,showline=True),
            width=1000,
            height=500,
            template='plotly_dark')
fig=go.Figure(data=data,layout=layout)
fig.show()

data4=timesdata[timesdata['year']==2014].iloc[:2,:]
data4

"""barmode is relative
whenever you will be use  iplot(fig)  that time  dict is enough
whnever you will be use fig.show() ,go.Figure,go.Layout
"""

t=go.Bar(x=data4.country,y=data4.total_score,text=data4.income,name='n1',marker=dict(color='yellow'))
t1=go.Bar(x=data4.country,
          y=data4.international,text=data4.num_students,name='n2')
data=[t,t1]
layout=go.Layout(title="country vs international guys",barmode="relative",
            xaxis=dict(title='country',showgrid=True,showline=True),
            yaxis=dict(title='total_score',showgrid=True,showline=True),
            width=1000,
            height=500,
            template='plotly_dark')
fig=go.Figure(data=data,layout=layout)
fig.show()

#relative
Second Bar Charts Example: citations and teaching of top 3 universities in 2014 (style2)
Actually, if you change only the barmode from group to relative in previous example, you achieve what we did here. However, for diversity I use different syntaxes.

Import graph_objs as go
Creating traces
x = x axis
y = y axis
name = name of the plots
type = type of plot like bar plot
data = is a list that we add traces into it
layout = it is dictionary.
xaxis = label of x axis
barmode = bar mode of bars like grouped( previous example) or relative
title = title of layout
fig = it includes data and layout
iplot() = plots the figure(fig) that is created by data and layout



"""barmode relative"""



df2011=timesdata[timesdata['year']==2011].iloc[:4,:]
df2011

import plotly.graph_objects as go
from plotly.offline import iplot

# Assuming df2011 is your DataFrame with columns 'university_name' and 'income'
t1 = go.Pie(
    labels=df2011.university_name,
    values=df2011.income,
    pull=[0, 0.1, 0, 0],  # Pull the second slice (index 1) slightly
    textinfo='percent+label',  # Display percentage and label in the hover info
    hoverinfo='label+value+percent',  # Display label, value, and percentage in the hover info
    )  # Set the patterns for each slice


data = [t1]

# Create a layout (optional)
layout = go.Layout(title='Pie Chart Example')

# Create a figure
fig = go.Figure(data=data, layout=layout)

# Show the plot
fig.show()

df2014=timesdata[timesdata['year']==2014].iloc[:10,:]
df2014

df2014=timesdata[timesdata['year']==2014].iloc[:4,:]
df2014
t1=go.Pie(labels=df2014.university_name,values=df2014.income,pull=[0,0,0,0])
data=[t1]
layout=go.Layout(title_text="university names vs income")
fig=go.Figure(data=data,layout=layout)
fig.show()

df2012=timesdata[timesdata['country']=='United States of America'].iloc[:4,:]
df2012

box plot

Median (50th percentile) = middle value of the data set. Sort and take the data in the middle. It is also called 50% percentile that is 50% of data are less that median(50th quartile)(quartile)

25th percentile = quartile 1 (Q1) that is lower quartile
75th percentile = quartile 3 (Q3) that is higher quartile
height of box = IQR = interquartile range = Q3-Q1
Whiskers = 1.5 * IQR from the Q1 and Q3
Outliers = being more than 1.5*IQR away from median commonly.

trace = box
y = data we want to visualize with box plot
marker = color

import pandas as pd
import seaborn as sns

"""box plot in plolty graph object  boxmean is true we will get the mean
2.boxmean ='sd' it will gives mean and std
"""

t1=go.Box(y=[1,5,6,7,9,11,45,32,45,67,99],name='mean',marker=dict(color='darkblue'),line=dict(color='yellow'),boxmean=True,boxpoints='all')

t2=go.Box(y=[1,5,6,7,9,11,45,32,45,67,99,67],name='mean2',marker=dict(color='red'),boxmean='sd',boxpoints='all')
data=[t1,t2]
layout=go.Layout(title_text="box plot")
fig=go.Figure(data=data,layout=layout)
fig.show()

"""t2 boxpints is false it not display outliers and markers and lines"""

t1=go.Box(y=[2,5,6,78,99,45,67,90],boxpoints='all',marker=dict(color='red'),line=dict(color='green'),name='mean1',boxmean=True)

t2=go.Box(y=[2,5,6,78,99,45,67,90,104,120],boxpoints=False,marker=dict(color='black'),line=dict(color='green'),name='mean2',boxmean='sd')

t3=go.Box(y=[2,5,6,78,99,45,67,90,104,120,190],boxpoints='all',marker=dict(color='black'),line=dict(color='green'),name='mean3',boxmean='sd')

t4=go.Box(y=[2,5,6,78,99,45,67,90,104,120,190,200],boxpoints='suspectedoutliers',marker=dict(color='black'),line=dict(color='green'),name='mean4',boxmean='sd')

data=[t1,t2,t3,t4]
#fig=go.Figure(data=data,layout=layout)
fig.show()

x2015 = timesdata[timesdata.year == 2015]

trace0 = go.Box(
    y=x2015.total_score,
    name = 'total score of universities in 2015',
    marker = dict(
        color = 'rgb(12, 12, 140)',
    )
)
trace1 = go.Box(
    y=x2015.research,
    name = 'research of universities in 2015',
    marker = dict(
        color = 'rgb(12, 128, 128)',
    )
)
data = [trace0, trace1]
iplot(data)

"""box"""

data6=timesdata[timesdata['country']=='United States of America']

data6

t1=go.Box(y=data6.income,marker=dict(color='red'),name='total income of amecia',boxpoints='all',text=data6.university_name,boxmean=True)
t2=go.Box(y=data6.income,marker=dict(color='green'),name='university income of america1',boxpoints='all',text=data6.university_name,boxmean='sd')
data=[t1,t2]
layout=go.Layout(title_text="box plot",title_x=0.5)
fig=go.Figure(data=data,layout=layout)
fig.show()

data7=timesdata[timesdata['country']=='United Kingdom']
data7
t1=go.Box(y=data7.income,marker=dict(color='red'),name='university income',boxpoints='all',text=data6.university_name,boxmean=True)
t2=go.Box(y=data7.income,marker=dict(color='green'),name='university income1',boxpoints='all',text=data6.university_name,boxmean='sd')
data=[t1,t2]
layout=go.Layout(title_text="box plot",title_x=0.4)
fig=go.Figure(data=data,layout=layout)
fig.show()

data7=timesdata[timesdata['teaching']>=9.0].iloc[:10,:]
data7

data8=timesdata[timesdata['teaching']>=6.0].iloc[:15,:]
data8

Histogram
Lets look at histogram of students-staff ratio in 2011 and 2012 years.

trace1 = first histogram
x = x axis
y = y axis
opacity = opacity of histogram
name = name of legend
marker = color of histogram
trace2 = second histogram
layout = layout
barmode = mode of histogram like overlay. Also you can change it with stack

t1=go.Histogram(x=data7.teaching,marker=dict(color='red'),opacity=0.75,name='n1')

t2=go.Histogram(x=data8.income,marker=dict(color='blue'),opacity=0.75,name='n2')
data=[t1,t2]
layout=go.Layout(title_text="teaching",title_x=0.5,
                 xaxis=dict(title_text="teaching percentage",showline=True,showgrid=True)
                 )

fig=go.Figure(data=data,layout=layout)
fig.show()

x2011 = timesdata.student_staff_ratio[timesdata.year == 2011]
x2012 = timesdata.student_staff_ratio[timesdata.year == 2012]

trace1 = go.Histogram(
    x=x2011,
    opacity=0.75,
    name = "2011",
    marker=dict(color='rgba(171, 50, 96, 0.6)'))
trace2 = go.Histogram(
    x=x2012,
    opacity=0.75,
    name = "2012",
    marker=dict(color='rgba(12, 50, 196, 0.6)'))

data = [trace1, trace2]
layout = go.Layout(barmode='overlay',
                   title=' students-staff ratio in 2011 and 2012',
                   xaxis=dict(title='students-staff ratio'),
                   yaxis=dict( title='Count'),
)
fig = go.Figure(data=data, layout=layout)
iplot(fig)

timesdata



"""histogram

his
"""



"""histogram into form of barmode is overlay"""

t1=go.Histogram(x=[20,10,10,33,44,56,44,56,332,44,5,623,310,249,45],marker=dict(color='red'),name='n1')
t2=go.Histogram(x=[20,10,10,33,44,56,44,56,92,100,110,115,213,199,325,340,390,14,67,332,44,5,623,45],marker=dict(color='blue'),name='n2')


data=[t1,t2]
layout=go.Layout(title_text="histograms",barmode='overlay')
fig = go.Figure(data=data, layout=layout)
iplot(fig)

data10=timesdata[timesdata['year']==2014].iloc[:10,:]
data11=data10.sort_values(by="num_students",ascending=True)
data11

data12=timesdata[timesdata['year']==2012]
data13=data12.sort_values(by="teaching",ascending=False).iloc[:10,:]
data13

t1=go.Histogram(x=data11.teaching,name='n1',marker=dict(color='green'),text=data11.teaching)

t2=go.Histogram(x=data13.income,name='n2',marker=dict(color='black'),text=data13.income)
data=[t1,t2]
layout=go.Layout(title_text="histograms")
fig = go.Figure(data=data, layout=layout)
iplot(fig)

# import figure factory
import plotly.figure_factory as ff
import matplotlib.pyplot as plt
# data
dataframe = timesdata[timesdata.year == 2015]
data2015 = dataframe[["research","international", "total_score"]]

# add index feauture in data2015


# figure
# in parantesysis we have (our data, plot we want to see in the diagonal, index, color map,..)
fig = ff.create_scatterplotmatrix (data2015, diag='box', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)

plt.savefig('scatterplotmatrix.png')
plt.show()

import numpy as np
import matplotlib.pyplot as plt

data2015 = [10, 20, 30, 40, 50]  # Replace this with your actual data

# Assuming data2015 is a list
indices = np.arange(1, len(data2015)+1)

for value in indices:
    print(value)

# Plotting the data
plt.plot(indices, data2015, marker='o')
plt.xlabel('Index')
plt.ylabel('Data Value')
plt.title('Data in 2015')
plt.show()

# import figure factory
import plotly.figure_factory as ff

# data
dataframe = timesdata[timesdata.year == 2015]
data2015 = dataframe.loc[:,["research","international", "total_score"]]

# add index feauture in data2015
data2015 ['index'] = np.arange(1,len(data2015)+1)

# figure
# in parantesysis we have (our data, plot we want to see in the diagonal, index, color map,..)
fig = ff.create_scatterplotmatrix (data2015, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)
iplot(fig)
plt.savefig('scatterplotmatrix.png')
plt.show()

import plotly.express as px
px.scatter_matrix(timesdata,dimensions[['research','total_scores','num_students']])

"""in plotly express we will use few paramter symbol,labels are eliminates _ and diagonal_visiblke is False"""

fig = px.scatter_matrix(timesdata,
    dimensions=["student_staff_ratio","num_students","international","total_score","income"],symbol='year',labels={col:col.replace('_','') for col in timesdata.columns})

fig.update_traces(diagonal_visible=False)
fig.show()

fig = px.scatter_matrix(timesdata,
    dimensions=["student_staff_ratio","num_students","international","total_score","income"],symbol='year',labels={col:col.replace('_','') for col in timesdata.columns})


fig.show()

import plotly.figure_factory as ff

# data
dataframe = timesdata[timesdata.year == 2015]
data2015 = dataframe.loc[:,["research","international", "total_score"]]

# add index feauture in data2015
data2015 ['index'] = np.arange(1,len(data2015)+1)

# figure
# in parantesysis we have (our data, plot we want to see in the diagonal, index, color map,..)
fig = ff.create_scatterplotmatrix (data2015, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)
iplot(fig)
plt.savefig('scatterplotmatrix.png')
plt.show()

timesdata

timesdata

"""plotly.figure_factory.create_scatterplotmatrix"""

import plotly.figure_factory as ff

# data
dataframe = timesdata[timesdata.year == 2015]
data2015 = dataframe[["research","international", "total_score"]]

# add index feauture in data2015
data2015 ['index'] = np.arange(1,len(data2015)+1)

# figure
# in parantesysis we have (our data, plot we want to see in the diagonal, index, color map,..)
fig = ff.create_scatterplotmatrix (data2015, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)
iplot(fig)
plt.savefig('scatterplotmatrix.png')
plt.show()

timesdata1=timesdata[timesdata['year']==2011]
timesdata1

import plotly.figure_factory as ff
import matplotlib.pyplot as plt
timesdata=timesdata[timesdata['year']==2015]
data2015=timesdata.loc[:,["university_name","year","research"]]

data2015['index']=np.arange(1,len(data2015)+1)
fig = ff.create_scatterplotmatrix (data2015, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)



plt.savefig('scatterplotmatrix.png')
fig.show()

import pandas as pd

# Specify the encoding when reading the CSV file


# Specify the encoding when reading the CSV file
df = pd.read_csv('/content/Diwali Sales Data (1).csv', encoding='latin-1')


df

df2=df[df['State']=='Maharashtra']
df3=df2.loc[:,['Age','Orders']]
df3['index']=np.arange(1,len(df3)+1)
fig = ff.create_scatterplotmatrix (df3, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700)

fig.show()



timesdata

hotel=pd.read_csv("/content/hotel_bookings 2.csv")
hotel

hotel['reservation_status_date']=pd.to_datetime(hotel['reservation_status_date'],dayfirst=True)

hotel['year']=hotel['reservation_status_date'].dt.year

df4=hotel[hotel['year']==2014].iloc[:100,:]
df5=df4.loc[:,['lead_time','arrival_date_day_of_month','adults']]
df5['index']=np.arange(1,len(df5)+1)
fig=ff.create_scatterplotmatrix(df5, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700,title="scatterplotmatrix",size=10)
fig.show()

def create_scatterplotmatrix(df, index=None, endpts=None, diag='scatter', height=500, width=500, size=6, title='Scatterplot Matrix', colormap=None, colormap_type='cat', dataframe=None, headers=None, index_vals=None, **kwargs)

"""make plots"""

import plotly.graph_objects as go
from plotly.subplots import make_subplots

indian = df[df['Nationality']=='Indian']
overseas = df[df['Nationality']=='Overseas']

# create subplots
fig = make_subplots(rows=1, cols=2)
fig.add_trace(go.Scatter(x=indian['SR'], y=df['Runs'],
                         mode='markers', name='Indian'), row=1, col=1)
fig.add_trace(go.Scatter(x=overseas['SR'], y=overseas['Runs'],
                         mode='markers', name='Overseas'), row=1, col=2)
fig.update_layout(title="Runs vs Strike Rate")
fig.show()

timesdata12016=timesdata[timesdata['country']=='United States of America'].iloc[:10,:]
timesdata12016

timesdata12017=timesdata[timesdata['country']=='United Kingdom'].iloc[:20,:]
timesdata12017

"""scatter in make_subplots"""

import plotly.graph_objects as go
from plotly.subplots import make_subplots

indian = df[df['Nationality']=='Indian']
overseas = df[df['Nationality']=='Overseas']

# create subplots
fig = make_subplots(rows=1, cols=2)
fig.add_trace(go.Scatter(x=indian['SR'], y=df['Runs'],
                         mode='markers', name='Indian'), row=1, col=1)
fig.add_trace(go.Scatter(x=overseas['SR'], y=overseas['Runs'],
                         mode='markers', name='Overseas'), row=1, col=2)
fig.update_layout(title="Runs vs Strike Rate")
fig.show()

import plotly.graph_objects as go
from plotly.subplots import make_subplots
fig = make_subplots(rows=1, cols=2)
fig.add_trace(go.Scatter(x=timesdata12016.university_name,y=timesdata12016.income,mode='markers',name='n1'),row=1,col=1)
fig.add_trace(go.Scatter(x=timesdata12017.university_name,y=timesdata12017.income,mode='markers',name='n2'),row=1,col=2)
fig.update_layout(showlegend=True,title="university vs income",template='plotly_dark')

import plotly.graph_objects as go
from plotly.subplots import make_subplots

# Assuming timesdata12016 and timesdata12017 are your DataFrames

# Create subplots
fig = make_subplots(rows=1, cols=2)

# Define traces for the first subplot
t1 = go.Scatter(x=timesdata12016['university_name'], y=timesdata12016['income'], mode='markers', name='n1')

# Define traces for the second subplot
t2 = go.Scatter(x=timesdata12017['university_name'], y=timesdata12017['income'], mode='markers', name='n2')

# Add traces to the subplots
fig.add_trace(t1, row=1, col=1)
fig.add_trace(t2, row=1, col=2)

# Add titles to each subplot
fig.update_layout(
    template='plotly_dark',
    annotations=[
        dict(text='Title for Plot 1', x=0.05, y=1.05, xref='paper', yref='paper', showarrow=False),
        dict(text='Title for Plot 2', x=0.55, y=1.05, xref='paper', yref='paper', showarrow=False),
    ],
    showlegend=True
)

# Show the plot
fig.show()

import plotly.graph_objects as go
from plotly.subplots import make_subplots
fig = make_subplots(rows=1, cols=2)
fig.add_trace(go.Scatter(x=timesdata12016.num_students,y=timesdata12016.income,mode='lines',name='n1'),row=1,col=1)
fig.add_trace(go.Scatter(x=timesdata12017.num_students,y=timesdata12017.income,mode='lines',name='n2'),row=1,col=2)
fig.update_layout(showlegend=True,title="university vs income",template='plotly_dark',width=1000,height=1000,title_x=0.5,title_y=1.0)

timesdata2015=timesdata[timesdata['year']==2015].iloc[:20,:]

fig=make_subplots(rows=1,cols=2)
fig.add_trace(go.Bar(x=timesdata2015['country'].value_counts().index,y=timesdata2015['country'].value_counts().values,name='n1'),row=1,col=1)
fig.add_trace(go.Bar(x=timesdata2015['university_name'].value_counts().index,y=timesdata2015['university_name'].value_counts().values,name='n2'),row=1,col=2)


fig.update_layout(title_text="country vs university ",title_x=0.5,title_y=0.5,
                  height=500,width=1000)

hotel1=hotel[hotel['year']==2014].iloc[:40,:]
hotel1

hotel1=hotel[hotel['year']==2014].iloc[:40,:]
hotel1
fig=make_subplots(rows=1,cols=2)
fig.add_trace(go.Bar(x=hotel1['customer_type'].value_counts().index,
                     y=hotel1['customer_type'].value_counts().values,
                     name=1),row=1,col=1)
fig.add_trace(go.Bar(x=hotel1['arrival_date_day_of_month'].value_counts().index,
                     y=hotel1['arrival_date_day_of_month'].value_counts().values,
                     name=2),row=1,col=2)
fig.add_annotation(
    text="This is a text annotation for the first plot",
    xref="paper",
    yref="paper",
    x=1.5,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)

fig.add_annotation(
    text="This is a text annotation for the second plot",
    xref="paper",
    yref="paper",
    x=1.0,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)



fig.update_layout(title_text="customer type vs arrival date of month",title_x=0.5,width=1000,height=500,template='seaborn')

fig.show()

import plotly.graph_objects as go
from plotly.subplots import make_subplots

# Assuming 'hotel' is your DataFrame

# Select a subset of data for the year 2014
hotel1 = hotel[hotel['year'] == 2014].iloc[:40, :]

# Create subplots
fig = make_subplots(rows=1, cols=2)

# Add the first bar chart to the subplot
fig.add_trace(
    go.Bar(
        x=hotel1['customer_type'].value_counts().index,
        y=hotel1['customer_type'].value_counts().values,
        name='Customer Type',
    ),
    row=1,
    col=1
)

# Add the second bar chart to the subplot
fig.add_trace(
    go.Bar(
        x=hotel1['arrival_date_day_of_month'].value_counts().index,
        y=hotel1['arrival_date_day_of_month'].value_counts().values,
        name='Arrival Day of Month',
    ),
    row=1,
    col=2
)

# Add text annotations
fig.add_annotation(
    text="This is a text annotation for the first plot",
    xref="paper",
    yref="paper",
    x=0.5,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)

fig.add_annotation(
    text="This is a text annotation for the second plot",
    xref="paper",
    yref="paper",
    x=1.5,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)

# Update layout
fig.update_layout(
    title_text="Customer Type vs Arrival Date of Month",
    title_x=0.5,
    width=1000,
    height=500,
    template='seaborn'
)

# Show the plot
fig.show()

hotel1=hotel[hotel['hotel']=='Resort Hotel']
fig=make_subplots(rows=2,cols=2)
fig.add_trace(go.Scatter(x=hotel1.lead_time,y=hotel1.arrival_date_day_of_month,mode='markers',name='x1',marker=dict(color='red')),row=1,col=1)
fig.add_trace(go.Scatter(x=hotel1.lead_time,y=hotel1.arrival_date_week_number,mode='markers',name='x2',marker=dict(color='green')),row=1,col=2)
fig.add_trace(go.Scatter(x=hotel1.lead_time,y=hotel1.adr,name='x3',mode='markers',marker=dict(color='white')),row=2,col=1)
fig.add_trace(go.Scatter(x=hotel1.lead_time,y=hotel1.arrival_date_day_of_month,name='x4',mode='markers',marker=dict(color='yellow')),row=2,col=2)
fig.add_annotation(
    text="This is a text annotation for the first plot",
    xref="paper",
    yref="paper",
    x=0.5,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)

fig.add_annotation(
    text="This is a text annotation for the second plot",
    xref="paper",
    yref="paper",
    x=1.5,
    y=1.15,
    showarrow=False,
    font=dict(size=12, color="black"),
)

# Update layout
fig.update_layout(
    title_text="Customer Type vs Arrival Date of Month",
    title_x=0.5,
    width=1000,
    height=500,
    template='plotly_dark'
)

# Show the plot
fig.show()

hotel1.head(2)

for col in hotel.describe(exclude='number').columns:
  print(col)

hotel1.year.value_counts()

"""bar we will cover x,y ,marker,name,text,width,height,almost parameter"""

fig=make_subplots(rows=4,cols=2)

fig.add_trace(go.Bar(x=hotel1.hotel,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='red'),name='n1'),row=1,col=1)

fig.add_trace(go.Bar(x=hotel1.meal,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='pink'),name='n2'),row=1,col=2)

fig.add_trace(go.Bar(x=hotel1.country,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='green'),name='n3'),row=2,col=1)

fig.add_trace(go.Bar(x=hotel1.deposit_type,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='black'),name='n4'),row=2,col=2)

fig.add_trace(go.Bar(x=hotel1.reservation_status,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='white'),name='n5'),row=3,col=1)

fig.add_trace(go.Bar(x=hotel1.customer_type,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='black'),name='n6'),row=3,col=2)

fig.add_trace(go.Bar(x=hotel1.market_segment,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='yellow'),name='n7'),row=4,col=1)
fig.add_trace(go.Bar(x=hotel1.distribution_channel,
              y=hotel1.adr,text=hotel1.adr,marker=dict(color='blue'),name='n8'),row=4,col=2)




fig.update_layout(title_text="adr vs different categorical types",title_x=0.5,showlegend=True,

                  width=1000,
                  height=1000,)

city=hotel[hotel['hotel']=='City Hotel'].iloc[:10,:]
city

hotel.head(2)

df

"""box

here the box  in make subplots we will get data from different tables like df,hotel,timesdata
"""



hotel1
fig=make_subplots(rows=4,cols=2)
fig.add_trace(go.Box(y=hotel1.adr,name='first box b1'),row=1,col=1)
fig.add_trace(go.Box(y=city.adr,name='second box plot b2'),row=1,col=2)
fig.add_trace(go.Box(y=df.Age,name='thrid box b1'),row=2,col=1)
fig.add_trace(go.Box(y=city.stays_in_week_nights,name='forth box plot b2'),row=2,col=2)
fig.add_trace(go.Box(y=df.Amount,name='fifith box b1'),row=3,col=1)
fig.add_trace(go.Box(y=city.adr,name='sixth box plot b2'),row=3,col=2)
fig.add_trace(go.Box(y=hotel1.adr,name='seven box b1'),row=4,col=1)
fig.add_trace(go.Box(y=city.adr,name='eight box plot b2'),row=4,col=2)
fig.update_layout(title_text="box plot",title_x=0.5,height=1000,width=1000)
fig.show()

df.head(2)

"""box in make _subplots allow tobe features like x or y,marker,name,boxpints,boxmean"""

'''df['State'].head(20)

state1=['Andhra Pradesh','Uttar Pradesh','Maharashtra','Delhi','Gujrat','Karnataka']
AP=df[df['State']=='Andhra Pradesh']
UP=df[df['State']=='Uttar Pradesh']
MP=df[df['State']=='Maharashta']
D=df[df['State']=='Delhi']
G=df[df['State']=='Gurjat']
K=dMOf[df['State']=='Karnataka']'''

fig=make_subplots(rows=1,cols=4)
fig.add_trace(go.Box(y=df.Amount,name='amount of cracker',boxpoints='all',boxmean=True,marker=dict(color='green')),row=1,col=1)
fig.add_trace(go.Box(y=hotel1.adr,name='average rate daily',boxpoints='all',boxmean='sd',marker=dict(color='red')),row=1,col=2)
fig.add_trace(go.Box(y=city.adr,name='average rate daily',boxpoints='outliers',boxmean='sd'),row=1,col=3)
fig.add_trace(go.Box(y=hotel.adr,name='average rate daily',boxpoints=False,boxmean='sd'),row=1,col=4)

"""in make_subplots there is no bins and text allow in histogram"""

fig=make_subplots(rows=1,cols=2)
fig.add_trace(go.Histogram(x=city.stays_in_week_nights,name='n1',text=city.stays_in_week_nights),row=1,col=1)

fig.add_trace(go.Histogram(x=city.stays_in_weekend_nights,marker=dict(color='green'),text=city.stays_in_week_nights,name='n2'),row=1,col=2)
fig.update_layout(title="city in stay weeks nights  vs stay weekend night ",title_x=0.5,
                  height=700,width=1000,showlegend=True)

hotel1
fig=make_subplots(rows=4,cols=3)
fig.add_trace(go.Box(y=hotel1.adr,name='first box b1'),row=1,col=1)
fig.add_trace(go.Box(y=city.adr,name='second box plot b2'),row=1,col=2)
fig.add_trace(go.Bar(x=hotel.hotel,y=hotel.adr,name='thrid box plot b3'),row=1,col=3)
fig.add_trace(go.Box(y=df.Age,name='fourth box b1'),row=2,col=1)
fig.add_trace(go.Box(y=city.stays_in_week_nights,name='fifth box plot b2'),row=2,col=2)
fig.add_trace(go.Bar(x=city.customer_type,y=city.adr,name='sixth box plot b2'),row=2,col=3)
fig.add_trace(go.Box(y=df.Amount,name='seven box b1'),row=3,col=1)
fig.add_trace(go.Box(y=city.adr,name='eight box plot b2'),row=3,col=2)
fig.add_trace(go.Bar(x=city.hotel,y=city.adr,name='nine box plot b2'),row=3,col=3)
fig.add_trace(go.Box(y=hotel1.adr,name='ten box b1'),row=4,col=1)
fig.add_trace(go.Box(y=city.adr,name='eleven box plot b2'),row=4,col=2)
fig.add_trace(go.Bar(x=city.reservation_status,name='twleve box plot b2'),row=4,col=3)
fig.update_layout(title_text="box plot",title_x=0.5,height=1000,width=1000)
fig.show()

df5=hotel[hotel['year']==2014].iloc[:100,:]
df2016=df5.loc[:,['lead_time','arrival_date_day_of_month','adults']]
df2016['index']=np.arange(1,len(df2016)+1)
fig=ff.create_scatterplotmatrix(df2016, diag='box', index='index', colormap='Portland',
                                  colormap_type='cat', height=700, width=700,title="scatterplotmatrix",size=10)
fig.show()

city1=hotel[hotel['year']==2016].iloc[:40,:]
city2=city1[['adr','stays_in_weekend_nights','year']]
city2['index']=np.arange(1,len(city2)+1)
ff.create_scatterplotmatrix(city2,index='index',diag='histogram',colormap='Portland',colormap_type='cat',height=1000,width=1000,title="scatter plot marix in plolty",size=10)

fig=px.scatter_matrix(hotel,dimensions=['year','adults','adr'],symbol='year',labels={col:col.replace('_','') for col in hotel.columns})
fig.update_traces(diagonal_visible=False)
fig.show()

import plotly.graph_objects as go
from plotly.subplots import make_subplots
import numpy as np

# Create some sample data
np.random.seed(42)
data1 = np.random.randn(1000)
data2 = np.random.randn(1000)

# Create subplots with two rows and one column
fig = make_subplots(rows=2, cols=1)

# Add histograms to the subplots
fig.add_trace(go.Histogram(x=data1, name='Histogram 1'), row=1, col=1)
fig.add_trace(go.Histogram(x=data2, name='Histogram 2'), row=2, col=1)

# Update layout
fig.update_layout(
    title_text="Histogram Subplots",
    title_x=0.5,
    showlegend=True,
    height=600,
    width=800
)

# Show the plot
fig.show()

teams_title = ['Royal Challengers Bangalore','Kings Xi Punjab','Mumbai Indians',
        'Kolkata Knight Riders','Delhi Capitals','Rajasthan Royals',
        'Chennai Super Kings','Sunrisers Hyderabad']

rcb = df[df['Team']=='Royal Challengers Bangalore']
kxp = df[df['Team']=='Kings Xi Punjab']
mi = df[df['Team']=='Mumbai Indians']
kkr = df[df['Team']=='Kolkata Knight Riders']
dc = df[df['Team']=='Delhi Capitals']
rr = df[df['Team']=='Rajasthan Royals']
csk = df[df['Team']=='Chennai Super Kings']
srh = df[df['Team']=='Sunrisers Hyderabad']

fig = make_subplots(rows=4, cols=2, subplot_titles=teams_title)
fig.add_trace(go.Scatter(x=rcb['SR'], y=rcb['Runs'], mode='markers'), row=1, col=1)
fig.add_trace(go.Scatter(x=kxp['SR'], y=kxp['Runs'], mode='markers'), row=1, col=2)
fig.add_trace(go.Scatter(x=mi['SR'], y=mi['Runs'], mode='markers'), row=2, col=1)
fig.add_trace(go.Scatter(x=kkr['SR'], y=kkr['Runs'], mode='markers'), row=2, col=2)
fig.add_trace(go.Scatter(x=dc['SR'], y=dc['Runs'], mode='markers'), row=3, col=1)
fig.add_trace(go.Scatter(x=rr['SR'], y=rr['Runs'], mode='markers'), row=3, col=2)
fig.add_trace(go.Scatter(x=csk['SR'], y=csk['Runs'], mode='markers'), row=4, col=1)
fig.add_trace(go.Scatter(x=srh['SR'], y=srh['Runs'], mode='markers'), row=4, col=2)
fig.update_layout(title="Runs vs Strike Rate",showlegend=False)
fig.show()

city